...
func TestSDK_HTTPRedis_NextID(t *testing.T) {
	...
	// 创建HTTP服务器
	server, err := shortid.NewHTTPServer(":0", redisAddr, shortid.BusinessOrder) // :0 表示自动分配端口
	if err != nil {
	...
	for i := 0; i < count; i++ {
		...
		var response shortid.IDResponse
...
func TestSDK_HTTPRedis_Health(t *testing.T) {
	...
	server, err := shortid.NewHTTPServer(":0", redisAddr, shortid.BusinessOrder)
	if err != nil {
		t.Fatalf("Failed to create HTTP server: %v", err)
	}
	defer func() {
