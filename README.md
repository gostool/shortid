...
- 单实例存在算法上限：当前位宽下，单 machineID 每 10ms 最多 128 个序列，即约 `12,800 QPS`。
- 实测单实例 `11,678 QPS` 已接近该上限，所以 `50,000 / 11,678 ≈ 4.28s`，属于正常结果。
- 64 实例并发是横向扩展，总上限约 `12,800 * 64 = 819,200 QPS`；实测 `759,904 QPS`，接近总上限。
- 因此“64 实例更快”本质是并行放大总吞吐，不是单实例退化。
